...
| 摄像头质量 [0~63] | `camera_config` (0x78) | 同上，只改质量 |
...
板子到 PC 的链路上限约 **60~90 KB/s**，所以帧率基本由分辨率决定
（直连板子、不经 Scratch 测的）：

| 分辨率 | 帧大小 (质量 20) | 帧率 |
| --- | --- | --- |
| QVGA 320x240 | ~8 KB | **7~10 帧/秒** |
| VGA 640x480 | ~18 KB | **3~4 帧/秒** |

想更流畅就用 QVGA + 把质量调到 25~30（帧更小）。这条链路上限本身就是两个
固件参数决定的，见 [camera-debug-notes.md](camera-debug-notes.md) 里
"流式播放的带宽" 那一节：lwIP 的发送窗口 + 主循环每轮发多少片。
