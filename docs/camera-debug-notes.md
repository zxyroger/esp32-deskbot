# OV2640 摄像头排障记录 (2026-09-20)

## 现状: 已经能出图 (2026-09-21)

先看文末的 [收尾](#收尾-20260921-摄像头已经能出图了) 一节 —— 那里是最终结论和改法。
中间几节是排查过程, 其中"D4/D5 没有信号"的说法**后来被推翻了**: 那是用 1ms 固定
窗口量出来的假象 (OV2640 的 JPEG 输出是一阵一阵的), 换成"数到足够多边沿才收工"
之后, 8 根数据线全都有数据。真正的病根是:

* AXP2101 的摄像头供电冷启动只有一路开 (SCCB 写得进但传感器内部没上电);
* XCLK 被降到 10MHz 时, 传感器生成的 JPEG 头里宽高是错的;
* JPEG 帧缓冲按 `w*h/5` 只有 61KB, 细节多时整帧判 `FB-OVF` 丢掉;
* esp32-camera 的 `cam_hal.c` 要求帧头必须在第 0 字节, 而本板每次连拍的头一两帧
  常常从半个帧开始 → 永远 `NO-SOI`, 驱动把每一帧都丢掉。

处理完这四条以后, VGA 640x480 每帧约 78KB, 拍的 `FF D8 ... FF D9` 完整。

## 是怎么量出来的

menuconfig -> `摄像头 (OV2640 DVP)` -> `TMX_CAMERA_PIN_PROBE` = y, 开机日志会打:

```
W tmx_camera: 探针 PCLK  (IO16):    1738 沿/1   ms =  1738000 Hz  不过滤  [被驱动为高]
W tmx_camera: 探针 VSYNC (IO3 ):       2 沿/200 ms =       10 Hz  滤波1us [被驱动为低]
W tmx_camera: 探针 HREF  (IO46):     850 沿/20  ms =    42500 Hz  滤波1us [悬空/没被驱动]
W tmx_camera: 探针 D0    (IO7 ):     129 沿/1   ms =   129000 Hz  不过滤  [悬空/没被驱动]
W tmx_camera: 探针 D1    (IO5 ):      54 沿/1   ms =    54000 Hz  不过滤  [被驱动为低]
W tmx_camera: 探针 D2    (IO4 ):     251 沿/1   ms =   251000 Hz  不过滤  [悬空/没被驱动]
W tmx_camera: 探针 D3    (IO6 ):      37 沿/1   ms =    37000 Hz  不过滤  [悬空/没被驱动]
W tmx_camera: 探针 D4    (IO15):       0 沿/1   ms =        0 Hz  不过滤  [被驱动为低]
W tmx_camera: 探针 D5    (IO17):       0 沿/1   ms =        0 Hz  不过滤  [被驱动为低]
W tmx_camera: 探针 D6    (IO18):     204 沿/1   ms =   204000 Hz  不过滤  [被驱动为低]
W tmx_camera: 探针 D7    (IO9 ):     106 沿/1   ms =   106000 Hz  不过滤  [被驱动为高]
W tmx_camera: === 全引脚扫描: 每根 1ms, 只列有活动的脚 ===
W tmx_camera: === 扫描结束 ===
```

怎么读这几行:

* 边沿数是 **硬件 PCNT 计数器**数的 (软件采样在 MHz 量级会混叠, 数字不可信)。
* 方括号里的是"内部上拉/下拉各测一次"的结果: 两次都读 0 = 这根脚被外部低阻钳死在低电平;
  两次都读 1 = 被钳死在高电平; 跟着电平走 = 悬空/没被驱动 (或者信号在跳)。
* **全引脚扫描**把板上没被外设占用的 GPIO (0/10/11/22/23/24/25) 全量了一遍, 一根都没信号 ——
  所以这两个 bit 不是"被走到别的 IO 上了" (那样改 Kconfig 就能救), 而是这两根线本身的问题:
  断线 / 短到地 / 模组或连接器上那两个脚虚焊。
* 摄像头断电 (AXP2101 两路 LDO 关掉) 再上电时探针照样会跑: 8 根数据线全变"被驱动为低"
  (没供电的 CMOS 输入被内部上拉经 ESD 二极管钳住), 可以用这一步区分问题在模组还是板子。

## 下一步 (要动硬件)

1. 重插 / 换一根摄像头排线, 检查连接器和模组这一侧的 `IO15_DVP_D4` / `IO17_DVP_D5`
   有没有虚焊、短路、连锡 (这两根在探针里是唯一 0 沿的)。
2. 想区分"板子短了"还是"模组/排线拉的": 把摄像头模组拔掉再上电。固件现在在摄像头
   初始化失败时**也会跑探针**, 所以日志里照样有那十几行:

   * IO15/IO17 还是"被驱动为低" -> 问题在板子这一侧;
   * 变成"悬空/没被驱动" -> 是模组/排线那侧把它们拉住的。
3. 有条件就换一块 OV2640 模组复测, 只看这两行即可判定。

## 供电 (AXP2101) —— 已修好

* 本板 OV2640 的 AVDD -> BLDO1、DVDD -> BLDO2, 由 AXP2101 (I2C `0x34`) 供电。
  芯片上电默认只开 BLDO1 (`0x90 = 0x55`), DVDD 那一路是关的。这时 SCCB 还能认出
  `0x30`, 但寄存器写会 NACK / 读回错值, 日志是:

  ```
  E sccb-ng: SCCB_Write Failed addr:0x30, reg:0x41, data:0x24, ret:259
  E camera: Camera probe failed with error 0xffffffff(ESP_FAIL)
  ```

  摄像头完全起不来 (冷启动后必现, 只有先用 PC 脚本把两路打开才行)。
* 固件现在会在初始化摄像头之前把 `0x90` 的 bit4/bit5 都置 1
  (Kconfig `TMX_CAMERA_PMIC_AXP2101`, 默认 y; **只动使能位, 不改电压**), 冷启动直接可用:

  ```
  I tmx_camera: PMIC AXP2101: LDO 使能 0x90 = 0x75 (BLDO1/AVDD=on, BLDO2/DVDD=on)
  I tmx_camera: OV2640 ready: VGA 640x480, JPEG 质量 12, XCLK 10MHz, fb=1, PID=0x26
  ```
* 手动看 / 改: `python tools\axp2101_power.py <板子IP>` (加 `--on` / `--off`,
  或 `--avdd 2800 --dvdd 1200`, 最后跟 `--reset` 让固件重新初始化摄像头)。
* 实测芯片默认 `BLDO1=1.8V / BLDO2=2.8V` 和 `2.8V / 1.2V` 两种组合都能出流 (PCLK 1.7MHz、
  VSYNC 10Hz)。固件不改电压, 具体用哪组请对着原理图确认。

## 常用自检命令

```powershell
python D:\esp\onegpio\tools\stop_s3extend.ps1          # 板子同时只服务一个客户端
python D:\esp\onegpio\tools\pc_camera_check.py 192.168.0.106 --info   # 看状态
python D:\esp\onegpio\tools\pc_camera_check.py 192.168.0.106          # 拍 1 帧
python D:\esp\onegpio\tools\axp2101_power.py 192.168.0.106            # 看 AXP2101 状态
python D:\esp\onegpio\tools\ov2640_regs.py 192.168.0.106              # 直接读传感器寄存器
C:\Espressif\tools\python\v5.5.4\venv\Scripts\python.exe tools\serial_tail.py COM14 --seconds 12 --filter "PMIC|OV2640|NO-SOI|探针|扫描"

## 引脚归属排查 (2026-09-20 晚): IO15/IO17 没被 UART / ADC / 32K 晶振占用

怀疑这两个 bit 是"被别的功能占了"时, 固件探针会先打三行只读的寄存器状态
(`PIN ... IOMUX=... / 矩阵 ... / RTC ...`)。ESP32-S3 上这三根脚的先天身份是:

| 脚 | IO MUX 寄存器名 | RTC pad |
| --- | --- | --- |
| IO15 (D4)  | `PERIPHS_IO_MUX_XTAL_32K_P_U` | `RTC_IO_XTAL_32P_PAD_REG` |
| IO16 (PCLK)| `PERIPHS_IO_MUX_XTAL_32K_N_U` | `RTC_IO_XTAL_32N_PAD_REG` |
| IO17 (D5)  | `PERIPHS_IO_MUX_DAC_1_U`      | `RTC_IO_PAD_DAC1_REG` |

实测 (摄像头拔掉、初始化失败时; 三根脚一模一样):

```
PIN D4   IO15 IOMUX=0x00001A02 MCU_SEL=1 IE=1 PU=0 PD=0
PIN D4   IO15 矩阵 out_sel=256(GPIO 输出寄存器) oen_sel=1 oen_inv=0 GPIO_OE=0
PIN D4   IO15 RTC XTAL_32K_P=0x40000000 mux=0 fun=0 ie=0 rue=0 rde=0
PIN PCLK IO16 ... (同上)
PIN D5   IO17 ... (同上)
```

逐条结论:

1. **不是 32.768kHz 晶振 (XTAL_32K_P)**: `RTC_IO_XTAL_32P_PAD` 的 `mux_sel = 0`
   → 这根 pad 处在"数字 GPIO"模式, 没有被 RTC 域接管; `fun_sel = 0`、`fun_ie = 0`、
   `rue = rde = 0` (RTC 侧的上下拉也全关)。固件里 `CONFIG_RTC_CLK_SRC_INT_RC=y`
   (RTC 时钟用内部 RC), 根本没启用外部 32K 晶振。
2. **不是 UART**: 工程里没有 `uart_set_pin`/`uart_driver_install`, 控制台走
   USB-Serial-JTAG (`CONFIG_ESP_CONSOLE_UART_NUM=-1`)。寄存器层面: IO MUX
   `MCU_SEL = 1` 就是"普通 GPIO", 不是 `U0RTS`/`U1TXD` 那种固定功能;
   GPIO 矩阵 `out_sel = 256` 指向自己的 GPIO 输出寄存器 (不是 UART 信号),
   `GPIO_OE = 0` (输出关闭) —— 没有任何外设在驱动这两根脚。
3. **不是 ADC**: 模拟输入只用了 ADC1 的 GPIO1~6 (Scratch 模拟引脚); 真被配成
   ADC/模拟功能时 IO MUX 会是 `MCU_SEL = 0` + `FUN_IE = 0`, 实测是 1/1。
4. 三根脚 (IO15/IO16/IO17) 在"摄像头拔掉"的状态下寄存器完全一致 → 板子这一侧
   没有把它们接到任何别的功能上; 插上模组后只有 IO15/IO17 变成"被驱动为低",
   所以低电平是模组/排线那一侧带过来的 (传感器 D4/D5 输出被拉死, 或模组/排线
   在这两个位置接的是 GND)。

> 踩坑记录: 解字段只能用移位 `(v >> XX_S) & XX_V`。IDF 的 `REG_GET_FIELD(reg, F)`
> 是宏里带 `REG_READ(reg)` 的 —— 把"读回来的值"传进去, 它会拿这个值当地址去读内存,
> 直接就 Guru Meditation (LoadProhibited)。
```
## 收尾 (2026-09-21): 摄像头已经能出图了

**硬件没问题** —— 是"供电 + XCLK + 帧缓冲大小 + 驱动过严的帧头检查"几件事叠在一起,
表现成"永远 NO-SOI, 一帧也拿不到"。

修好的地方 (按重要性):

1. **供电**: AXP2101 的 BLDO1/BLDO2 冷启动只有一路开 → 固件现在开机自己开
   (`TMX_CAMERA_PMIC_AXP2101`, 见上一节)。
2. **XCLK 回 20MHz**: 调试时被降到 10MHz。10MHz 下拍出来的 JPEG, 头里的宽高是
   `641x480` / `897x480` 这种错值; 回到 20MHz 后 5 张全是 `640x480`。
3. **JPEG 帧缓冲**: 自动算法 `w*h/5` 只有 61440 字节, 画面细节多时整个帧超过它,
   驱动直接判 `FB-OVF` 丢帧 → 改用本地指定大小 100000 字节
   (`CONFIG_CAMERA_JPEG_MODE_FRAME_SIZE_CUSTOM=y` + `CONFIG_CAMERA_JPEG_MODE_FRAME_SIZE=100000`),
   JPEG 质量默认从 12 调成 20 (VGA 帧约 78KB)。
4. **驱动"帧头必须在第 0 字节"的检查**: esp32-camera 的 `cam_hal.c` 只要在第一条 DMA
   数据里找不到 `FF D8 FF` 就丢帧并停止采集, 而本板每次开始连拍的**头一两帧**常常是
   从半个帧开始的 (VSYNC/HREF 与 JPEG 数据流的相位), 于是永远拿不到帧。
   本地补丁 (文件里标了 `local debug patch`): 改成只告警不丢帧, 由上层
   `tmx_camera.c` 的 `jpeg_align_frame()` 在整帧里找 `FF D8 FF` / `FF D9`,
   裁出干净的一段再发给 PC; 找不到就丢掉这一帧 (连续 3 帧对不齐才报错)。
   **注意: `managed_components/` 是组件管理器下载的, 重新拉组件会覆盖这个补丁, 要重打。**
5. PC 侧 `pc_camera_check.py`: 固件发的是 esp32-camera 的 `pixformat_t` (4 = JPEG),
   老代码按 3 = JPEG 判断, 会把好帧误报成"不是 JPEG" → 已改成 3/4 都认。

现在的用法 (板子同时只服务一个客户端, 先停掉网关):

```powershell
python D:\esp\onegpio\tools\stop_s3extend.ps1
python D:\esp\onegpio\tools\pc_camera_check.py 192.168.0.106 --frames 5   # 存 cam_000..004.jpg
python D:\esp\onegpio\tools\pc_camera_check.py 192.168.0.106 --view       # 拍完直接打开
```

实测: VGA 640x480, ~78KB/帧, 5/5 帧都是完整的 `FF D8 ... FF D9`。

> 建议一次拍多帧 (`--frames 3` 以上): 头一两帧常常对不齐会被丢掉, 后面的就干净了。

## 噪点问题 (2026-09-21): XCLK 24MHz

画面"很多噪点"是 **XCLK 频率不对**造成的, 跟硬件/接线无关:

* esp32-camera 里 OV2640 的寄存器表 (`ov2640_settings_cif/jpeg3/...`) 是按
  **24MHz XCLK** 调的 (表头注释就写着 `// 30fps@24MHz`), 传感器内部的 PLL/ADC 时序
  都按这个来。给 20MHz 甚至 10MHz, 传感器照样出图, 但**像素级噪点巨大**, 而且
  采样偶尔出错 —— JPEG 头里的标记字节会被采成邻居值 (`FF C4`→`FF C5`,
  `FF C0`→`FF C1`, 都是最低位翻转 = D0/IO7 那根线上的采样错), 于是大多数帧
  PC 侧根本解不开 (Pillow 报 `cannot handle 0-bit layers`)。
* 实测对比 (VGA, 质量 20, 5 帧):

  | XCLK | 能解码的帧 | 帧大小 | 高频能量 (噪点指标) |
  | --- | --- | --- | --- |
  | 20MHz | 1~2 / 5 | ~78KB | ~53 |
  | 24MHz | **5 / 5** | ~67KB | **~11** |

  高频能量 = 每个像素与四邻平均之差的均值, 越小越干净 (5~11 是有内容的正常画面,
  50 以上就是"满屏噪点")。帧变小也说明噪点少了 (噪点最费码流)。
* 结论: **XCLK 保持 24MHz** (`CONFIG_TMX_CAMERA_XCLK_FREQ_HZ=24000000`)。
  之前"降 XCLK 到 10~16MHz 去噪"的说法是错的 —— 那是把信号问题误当成时钟太快,
  实际反而更糟。

## "一半的开机是满屏噪点" 与开机自检 (2026-09-21 深夜)

压完 XCLK 之后又发现一个**跟开机运气有关**的毛病: 同样的固件, 有的开机拍出来是
正常画面, 有的开机拍出来 "一屏噪点、PC 侧谁也不能解码"。连开 4 次板子的实测:

```
第1次: C4/OK  C4/OK  C4/OK
第2次: C4/OK  C4/OK  C4/OK
第3次: C5/FAIL C5/FAIL C5/FAIL   <- 这次开机就是坏的
第4次: C4/OK  C4/OK  C4/OK
```

* 判据是 JPEG 头里的 `FF C4` (DHT) 被采成了 `FF C5` (DAC)(DAC 在 Huffman JPEG 里
  本来就不该出现), 说明**传感器初始化时少写进去几个寄存器 / DVP 采样错位**,
  跟接线无关; `Pillow`、Windows 自带解码器都打不开这种帧。
* 处理: 固件开机后**自己抓一帧自检**(`frame_header_ok()`: 头 700 字节里不能有
  `FF C5`, 且必须出现 `FF C4`), 不对就 `esp_camera_deinit()` + 重新
  `esp_camera_init()` 再来, 最多 3 次。加完以后连开 5 次板子, 10/10 帧全部
  `C4/OK`。

## 弱光噪点: 自动增益上限 (`TMX_CAMERA_GAIN_CEILING`)

晚上/弱光下 OV2640 的自动增益会一直加到上限, 画面就是"能看出来但很多噪点"。
固件现在初始化后会把增益上限压到 Kconfig 里设的值 (默认 3 = 16X, 原来是 128X),
自动曝光会自动用更长曝光补偿, 画面亮一些、颗粒感也会变。

* 想更干净: 调到 2 (8X) 或 1 (4X), 画面会暗一些;
* 想恢复原厂: 填 6 (128X);
* 改完要重新编译烧录 (在线调参的 `pc_camera_tune.py` 目前还没打通, 见下)。

## 真正的病根找到了: VDDCAM_3V3(ALDO2) 一直是关的 (2026-09-21 深夜)

拿万用表量三路电压时发现: AVDD=2.8V ✓、DVDD=1.2V ✓、**VDDCAM_3V3 只有 1.8V** ✗。
按板子确认, VDDCAM_3V3 接的是 **AXP2101 的 ALDO2**, 而它的使能位 (`0x90` bit1)
**从开机起就是 0** —— 量到的 1.8V 是 I/O 漏电把悬空的轨抬起来的假电压。
也就是说摄像头的 **I/O 供电一直是断的**:

* DVP 数据线的高电平只有 1.8V, 低于 ESP32-S3 的判高门限 (~2.5V) → 数据被读得
  时好时坏, 表现就是帧头找不到 `FF D8`、字节成对重复;
* SCCB 照样能通, 因为 I2C 是开漏 + 上拉, 不吃这个门限 —— 所以之前一直误判成
  "传感器采样错位 / 模组被过压弄坏了"。

修法: 固件开机会把 ALDO2 (=VDDCAM_3V3) 设成 `TMX_CAMERA_PMIC_ALDO2_MV`
(默认 **2800mV**) 并打开; 自检失败做掉电重来时也把它一起关掉再上电。

改完实测 (VGA):

```
I tmx_camera: PMIC: 摄像头供电 AVDD(BLDO1)=2800mV, DVDD(BLDO2)=1200mV, VDDCAM(ALDO2)=2800mV
I tmx_camera: PMIC AXP2101: LDO 使能 0x90 = 0x77 ...
I tmx_camera: 开机自检: 第 1 帧 JPEG 头正常
I tmx_camera: OV2640 ready: VGA 640x480, JPEG 质量 20, XCLK 24MHz, fb=1, PID=0x26
第 1/2/3 帧: 640x480, 能被 Pillow 正常解码
```

同时这一轮也把"空闲断电"修好了: 拍照前那次重新初始化不再被误判成失败
(之前 `tmx_camera_init()` 末尾总会断电, 拍照路径看到 `s_sensor_on=false` 就以为上电失败)。

**更正前几节的推测**: 模组没有坏, "采样错位"是 I/O 供电缺失造成的, 不是过压损伤。
(DVDD 挂 2.8V 确实会让它发烫, 那条已经改回 1.2V。)

## 夜间模式: 降 XCLK 换更长的曝光 (2026-09-22)

夜里那些横纹是**逐行噪声**, 根源在自动增益: 想在同一亮度下把增益压下来, 就得给
自动曝光更长的曝光时间, 而曝光时间上限由**帧时间**决定。于是把帧率降下来即可。

试过的两个寄存器**都没用**:

* `CLKRC (0x11, sensor 档)`: 写 0x01/0x03/0x07/0x0F 后, 帧率(PROBE VSYNC)和 PCLK
  都纹丝不动 —— OV2640 在 JPEG 模式下帧率由编码器决定, 不受这个分频影响;
* `R_DVP_SP (0xD3, DSP 档)`: 同样只影响很小的范围, 帧率不变。

(顺带确认了寄存器写入本身是通的: 写 `0x3C=0x33` 能原样读回。)

**真正管用的是 XCLK**: 24MHz→12MHz 后帧率跟着降一半, 曝光上限翻倍。

实测 (同一场景, 白天, 每档 2~3 帧平均):

| XCLK | 亮度 | 行间跳动(横纹指标) |
| --- | --- | --- |
| 24MHz (默认) | 129.1 | 2.57 |
| **12MHz** | 137.1 | **1.29** |
| 8MHz | 128.5 | 1.97 |

用法 (在线切换, 立刻生效; 重启后回到 Kconfig 默认值):

```powershell
python tools\pc_camera_tune.py 192.168.0.102 --set xclk 12   # 夜间: 帧率减半, 更干净更亮
python tools\pc_camera_tune.py 192.168.0.102 --set xclk 24   # 白天: 恢复 35fps
```

想让它成为开机默认值: 改 `CONFIG_TMX_CAMERA_XCLK_FREQ_HZ` 再编译。

> 更正: 早前"20MHz/10MHz 会让画面更噪"的结论**不成立** —— 那次是在 VDDCAM_3V3
> (ALDO2) 还没供电的状态下测的, 与 XCLK 无关。

## 横纹的结论: 逐行随机噪声, 用多帧平均压掉 (2026-09-23)

24MHz 和 12MHz(夜间模式) 拍出来的横纹**一模一样**, 所以再做几个判别:

* 帧内空间周期: 两种帧率下都是 ~150 行 —— 但那条是画面的平滑渐变(场景本身),
  不是条纹周期; 真正的条纹是 **2~4 行一条**的细纹;
* 逐行起伏幅度 (相邻行均值之差的中位数): 亮区 0.32 / 中区 0.25 / **暗区 2.58**
  —— 暗部最明显, 两种帧率下基本持平;
* 帧与帧之间的相关性: +0.15 / +0.07 / +0.38 (24MHz), +0.09 / -0.25 / -0.28 (12MHz)
  → **每帧都换一个样子 = 随机时间噪声**, 不是固定图案、也不是灯光 100Hz 频闪
  (频闪的周期会随帧率变, 这里没变)。

传感器自带的降噪开关也用不上: `set_denoise` / `set_sharpness` 在 esp32-camera 的
OV2640 驱动里是**空实现**(直接返回 -1), 只有 `set_bpc` / `set_wpc` / `set_lenc`
有实现(坏点/镜头校正, 对行噪声没用)。

### 办法: 多帧平均 (`tools/pc_camera_merge.py`)

既然是每帧独立的随机行噪声, 拍同一静止场景 N 帧对齐平均就能把噪声降到 **1/√N**。
实测 (12 帧, 暗区):

| | 暗区逐行抖动 | 像素级噪点 |
| --- | --- | --- |
| 单帧 | 1.67 ~ 3.05 | 0.90 ~ 1.04 |
| **12 帧平均** | **0.60** | **0.59** |

```powershell
# 一步到位: 拍 12 帧再合并 (输出 PNG, 不二次压缩)
python tools\pc_camera_merge.py --host 192.168.0.102 --frames 12 --out tools\merged.png

# 也可以合并已经拍好的图
python tools\pc_camera_merge.py --files tools\avgframe_*.jpg --out tools\merged.png
```

代价: 只适合静止画面 (动的东西会糊), 而且要连拍 N 帧。4 帧降一半, 16 帧降到 1/4。

## 还没解决的

1. **串口日志开机后一段时间会停**: USB-Serial-JTAG 的日志通道会卡住 (TCP 命令
   照常执行), 复位一次就好。怀疑是某个任务在日志上阻塞把日志锁住了, 需要单独查。
2. ~~**在线调参命令 (0x7D) 没生效**~~ **已确认是通的** (2026-09-23): 用必然可观测的
   `--set xclk 12` 验证过, `--info` 立刻变成 `XCLK 12MHz`, 改回 24 也生效;
   `--quality` 改完帧大小跟着变 (质量 40 约 10KB / 质量 8 约 27KB)。
   之前觉得"没生效"的两种情况是:

   * `set_hmirror` / `set_vflip` 在 **JPEG 模式下本来就是空操作** (驱动只写 REG04 的
     镜像位, JPEG 数据通路绕过它) —— 实测 `--set hmirror 1` 后画面相关系数 0.996;
   * `aec_value` 在自动曝光还开着时会被自动算法覆盖, 看着"没变"。

## 摄像头发烫 (2026-09-21): 供电电压 + 空闲断电

摸上去很烫, 查出来是两件事:

1. **供电电压之前被留在 `2.8V / 2.8V`**: OV2640 的 DVDD 是 **1.2V 内核供电**,
   挂在 2.8V 上属于过压, 会明显发热 (时间长了也可能伤到传感器)。之前调试供电映射时
   为了做对照实验把 BLDO2 抬到 2.8V, 之后一直没改回来。
   固件现在**每次开机都把两路设成确定值**并打开:

   | Kconfig | 默认 | 接到 |
   | --- | --- | --- |
   | `TMX_CAMERA_PMIC_AVDD_MV` | 2800 | AVDD = BLDO1 |
   | `TMX_CAMERA_PMIC_DVDD_MV` | **1200** | DVDD = BLDO2 |

   开机日志会打: `PMIC: 摄像头供电 AVDD(BLDO1)=2800mV, DVDD(BLDO2)=1200mV`。

2. **初始化完就一直出流**: esp32-camera 一旦 init, 传感器就不停出图 (XCLK 一直跑,
   JPEG 编码器一直干活), 摸上去当然是温的。固件现在 `TMX_CAMERA_IDLE_POWER_OFF=y`
   (默认开):

   * 开机只做一次初始化自检, 随后**断电** (停 XCLK + PWDN 拉高);
   * 每次拍照前重新上电初始化 (~0.3s, 含一次 JPEG 头自检), 拍完 (或 CAMERA_STOP) 立刻断电;
   * 空闲时 SCCB 上 `0x30` 不应答, 模块是凉的 (用 `tools/ov2640_regs.py` 可以验证)。

   另外自检失败时, 重试现在会做一次**真正的掉电再上电** (PWDN 拉高 150ms 再拉低),
   因为只调 `esp_camera_init()` 清不掉"采样错位"那个状态。

## 横纹/噪点的真正病根: 上电后自动算法没收敛, 而固件每拍一张就断电 (2026-09-23)

前面几轮一直在往"供电纹波 / DVP 信号完整性"上查, 那些都不成立。真正的原因很朴素,
但被"空闲降温"这个设计放大了:

**OV2640 上电时 AEC / AGC / AWB 都是从默认值开始逐帧收敛的。头几帧模拟增益是拉满的、
白平衡也没收敛。而本固件为了降温每次拍照都要断电重上电 —— 于是每一张照片拿到的
恰好都是这个最脏的第 0 帧。**

### 实测 (VGA / 质量 12 / 同一会话连续取帧, 暗部)

| 帧序 | 平均亮度 | 行条纹 | 像素噪声 | 色偏 (R−Y) |
| --- | --- | --- | --- | --- |
| 第 0 帧 | 145.4 | **3.54** | **4.91** | 61.8 |
| 第 1 帧 | 131.3 | 2.91 | 4.96 | 52.3 |
| 第 2 帧 | 125.7 | 2.39 | 4.14 | 48.2 |
| 第 3 帧 | 122.3 | 1.56 | 3.25 | 46.7 |
| 第 5 帧 | 123.4 | 0.97 | 2.27 | 46.3 |
| 第 8 帧 | 129.3 | 0.73 | 2.11 | 50.6 |
| 第 10~19 帧 | ~129.3 | **0.62~0.82** | **1.92~2.16** | ~50.8 |

第 0 帧的逐行噪声是收敛后的 **4.5~8 倍**, 白平衡还差 10 个灰阶。人眼看到的就是
"很多噪点 + 彩色横纹 + 局部色带"。

### 怎么修

初始化之后、自检和拍照之前先抓一批帧丢掉不返回, 等自动算法收敛:

* `TMX_CAMERA_WARMUP_MS` (默认 **2000**, 0 = 关)
* `TMX_CAMERA_WARMUP_FRAMES` (默认 **20**, 兜底帧数: 夜间模式帧率减半时防止时间够了帧不够)

改完实测, **第一张照片**就到位了:

| | 行条纹 | 像素噪声 | 色偏 |
| --- | --- | --- | --- |
| 修复前第 0 帧 | 3.54 | 4.91 | 61.8 |
| 修复后第 0 帧 | **0.44** | **1.69** | **44.0** |

代价: 每次拍照多 2 秒 (只在拍照前, 空闲照样是断电的)。

### 为什么之前那些判断都不成立

在找到"帧序"这条线之前, 已经把常见的都排掉了。留个记录, 免得以后又绕回去:

* **不是 DVP 时序/信号完整性**: 24MHz 和 12MHz XCLK 下条纹指标一样。采样余量不足的话,
  时钟减半会让误码率大幅变化。
* **不是灯光频闪 (50Hz 市电)**: 逐行剖面的垂直频谱里**没有周期分量**, 最强分量就是
  画面自身的低频渐变; 而且帧率减半后条纹完全不变 —— 频闪的条纹位置必然随行读出速率改变。
* **不是电源纹波/串扰**: 逐行剖面的**帧间相关性 ≈ 0** (+0.07 / −0.14)。固定图案的纹波
  会给出接近 1 的相关性。
* **不是带宽/缓冲溢出**: 帧头 `FF D8 FF`、`FF D4`、`FF D9` 都正常, 帧 100% 能解出来。
  JPEG 码流被位错污染会出现大面积色块/解码失败, 而不是 1~4 个灰阶的细纹。
* **不是传感器坏了/模组个体差异**: 同一颗传感器收敛后就很干净, 说明硬件是好的。
* 顺带确认: `hmirror` / `vflip` 在 JPEG 模式下**不生效**(驱动写 REG04 的 HFLIP 位,
  但 JPEG 数据通路绕过了它) —— 实测 `--set hmirror 1` 后画面相关系数 0.996, 等于没变。
  这是驱动限制, 不是我们这边的问题。

### 复现/验证用的工具

`tools/pc_camera_stripe.py` (只依赖 Pillow + numpy, 用 IDF 的 Python 跑) 把
"亮度条 / 色度条 / 明区暗区 / 频谱周期 / 帧间相关性"一次算出来, 用来区分
"行噪声"和"场景内容"、"随机噪声"和"固定图案":

```powershell
& C:\Espressif\tools\python\v5.5.4\venv\Scripts\python.exe tools\pc_camera_stripe.py "tools\stripe\*.jpg" --region dark
```

关键判据: **同一场景连拍多帧, 行条纹在多帧平均后应该按 1/√N 衰减** —— 衰减了就是随机
行噪声(传感器), 不衰减就是固定图案(纹波/串扰/场景本身)。

## 把摄像头接到 Scratch 积木 (2026-09-23 深夜)

固件侧的协议 (0x78~0x7B / 上报 0x0F~0x12) 早就有了, 缺的是网关和 Scratch 扩展这两段。
补上之后踩到两个坑, 现象都很像"板子/摄像头坏了", 记一下。

### 坑 1: 上报码 0x13 (引脚探针) 把网关的接收循环直接打死

telemetrix 的接收循环是这样派发的:

```python
report = packet[0]
await self.report_dispatch[report](packet[1:])     # 直接索引, 没有兜底
```

`report_dispatch` 里没注册的上报码 -> `KeyError` -> **整个接收循环退出**。而且
这个循环是个没人管的 asyncio task, 异常不会打到 stderr, 于是:

* 网关进程还活着;
* 到板子的 TCP 还是 ESTABLISHED;
* 板子串口里照常打 `拍照: 指定帧数` / `帧对齐: 收到 20620 字节` / `拍完`;
* 但 PC 侧**从此收不到板子任何上报**, 连 `0x7B` 摄像头状态都不回, 一行日志都没有。

当时的触发源是 `CONFIG_TMX_CAMERA_PIN_PROBE=y` 已经打开: 每次摄像头初始化都会扫一遍
DVP 各引脚, 顺带用 0x13 把探针结果上报给 PC —— 而网关没注册 0x13, 于是每次拍照前
那一下初始化就把它打死了。

两条一起做的修法:

1. **关掉引脚探针** (`CONFIG_TMX_CAMERA_PIN_PROBE=n`)。它本来就是排障用的, 开着还会
   让每次拍照多扫 40 个 GPIO、刷一堆 `GPIO number error`, 又慢又吵。
   顺带修了一个既存编译错误: 探针关掉时 `probe_report()` 没有定义, 但"在线改 XCLK"
   和"读寄存器"两处还在调它 -> `implicit declaration of function 'probe_report'`。
   现在探针关掉时给一个空实现。
2. **补丁 11**: 接收循环改成
   `handler = self.report_dispatch.get(report)`, 没有处理器就打印一条
   `unknown report id: N` 后 `continue`, 处理器抛异常也只丢掉这一条。
   这样以后再冒出新上报码, 也是"少收一条"而不是"整条链路静默死掉"。

### 坑 2: telemetrix 的 WiFi 读函数按"短读"写, 一帧 80 多片必错位

`socket_aio_transport.py`:

```python
async def read(self, num_bytes=1):
    buffer = await self.reader.read(num_bytes)    # asyncio: 只保证"最多 n 字节"
    return buffer
```

而接收循环是"先读 1 个字节当长度, 再读那么多字节"。数据跨 TCP 分段时
`read(n)` 会短读, 从此全体错位。小包(超声波/I2C/响度)一般一个 TCP 段就过去了,
所以一直没暴露; 摄像头一帧 VGA 要发 80 多片, 必然踩到。

**补丁 10** 把 `read()` 改成读满 `num_bytes` 再返回。

### 怎么快速判断是哪一类问题

不看网关日志光看现象很容易误判, 这几个观察点最省事:

| 观察 | 说明什么 |
| --- | --- |
| 串口里板子在 `拍照`/`帧对齐: 收到 N 字节`/`拍完` | 板子侧完全正常, 问题在 PC 侧 |
| 连 `camera_info` (0x7B) 都不回, 但网关进程和 TCP 都还在 | 接收循环已经死了 -> 坑 1 那一类 |
| `camera_info` 回得来、照片收不到 | 拼帧/分片错位 -> 坑 2 那一类 |
| 网关日志 `report N handler failed` / `unknown report id: N` | 就是坑 1, 看 N 是哪个上报码 |

板子串口本身也不完全可靠(开机一段时间后会卡, 见前面), 但 `ets_printf` 和 TCP 上报
一直是好的, 所以"板子到底有没有收到命令"优先看串口那几行 `tmx_camera` 日志。

### 验证工具

`tools/test_camera_blocks.py` 不打开 Scratch 就能验证整条网关链路 (发
`ip_address`/`camera_config`/`camera_snapshot`, 收 `camera_info`/`camera_frame_start`/
`camera_frame`/`camera_status`, 并把 JPEG 存盘 + 校验 SOI/EOI):

```powershell
# 用系统 Python (python_banyan / s3-extend 装在它下面)
C:\Program Files\Python313\python.exe tools\test_camera_blocks.py --host 192.168.0.103 --frames 2 --dir tools\photos
```

扩展本身的积木逻辑用 `node tools\test_esp32s3_camera.js` 跑 (假 Scratch + 假 WebSocket,
不需要编辑器也不需要板子)。

## 流式播放 (2026-09-23 深夜): 把"拍照积木"改成实时画面

相机积木一开始只有"拍一张变成造型"。要改成**流式播放**(舞台上一直动)之后,
扩展侧和固件侧各有一件事必须处理, 顺带把链路上限从 58 KB/s 提到 88 KB/s。

### 扩展侧: 不能每帧新建造型

最直接的做法是每帧 `vm.addCostume(...)` 挂一个新造型 —— 15 帧/秒跑一分钟就是
900 个造型, 编辑器会卡死。所以改成:

* 第一帧: `vm.addCostume` 建一个固定名字的「摄像头画面」造型;
* 之后每一帧: `runtime.renderer.updateBitmapSkin(costume.skinId, canvas, 2, center)`
  原地换贴图 —— 造型数量不涨、内存不涨。

代价是造型资源的 `assetId` 还停在第一帧, 所以**关闭摄像头时会再刷一次资源**
(`refreshVideoAsset`), 这样工程存档里存的是最后一帧而不是第一帧。

另外两个细节:

* 上一帧还在解码/画的时候, 新来的帧直接丢掉 (宁可掉帧也不要排队积压 ——
  积压会让画面越来越滞后);
* 点编辑器的停止按钮时, 扩展通过 `runtime.on('PROJECT_STOP_ALL')` 自动调
  `关闭摄像头`。不然指令停了、板子还在出图发热, 这个坑很容易忘。

### 固件侧: 链路上限原来是主循环每轮发多少片

实测(直连板子, 不经 Scratch, 质量 20, 间隔 20ms):

| | 改之前 | 改之后 |
| --- | --- | --- |
| QVGA 320x240 | 7.2 帧/秒 (58 KB/s) | **11.8 帧/秒 (88 KB/s)** |
| VGA 640x480 | 3.6 帧/秒 (66 KB/s) | 3.8 帧/秒 (86 KB/s) |

改的是 `CAM_CHUNKS_PER_POLL`(主循环每轮最多发几片): 16 → 96。原来写 16 是怕
"长时间占住主循环", 但 `tmx_core_send()` 本来就是整包发完才返回, 一轮只推 3.9KB
反而把带宽浪费在来回绕主循环上。现在一轮把整帧推出去。

### 踩过的坑: 把 lwIP 发送窗口调大反而更糟

先量了一下来回延迟: 一次 LOOPBACK 往返 **18ms**。配上 lwIP 默认的
`CONFIG_LWIP_TCP_SND_BUF_DEFAULT = 5760`, 算下来上限正好是 5.7KB/18ms ≈ 320KB/s,
看着像是"窗口太小卡住了"。于是把它抬到 32KB —— **结果更糟**:

* 板子串口里帧生成速度飙到 30 帧/秒 (每 30~45ms 一行 `帧对齐`);
* 但 PC 侧 8 秒只收到 3~5 帧, 而且连下一次连接都超时 (板子的 socket 卡住)。

这块板子**没有 PSRAM**, 相机已经占掉 100KB 帧缓冲 + 32KB DMA, 再给每条 TCP
连接 32KB 发送缓冲, 内存/分段池跟不上 —— 表现就是帧被发了一半就丢, PC 侧的
拼帧器永远等不到完整帧。已经回退成 5760, 只保留 `CAM_CHUNKS_PER_POLL` 的改动。

> 要再往上提速的话, 正确的方向是先开 PSRAM, 或者把 TCP 发送缓冲改成"动态分配 +
> 适度(12~16KB)"再实测, 而不是直接把默认值拉到几十 KB。

### 测量时要注意的两个坑

1. `pc_camera_check.py --stream` 的 `--interval` **默认 200ms**, 会先把板子限速到
   5 帧/秒 —— 测吞吐要显式写小一点 (比如 `--interval 20`)。
2. **TurboWarp 里如果还跑着「拍照」循环, 会和流式播放抢板子**: 每次单张拍照都会
   重新上电 + 2 秒预热, 把正在跑的连续流打断。测帧率时要先把编辑器里的项目停下来
   (或者先关掉 TurboWarp), 否则量出来的是两种命令互相打断的结果。

## 帧率优化 (2026-09-24): 把 3.7 帧/秒提到 14 帧/秒

接着上面那节继续调。原则先说清楚:

> **帧率 ≈ 板子到 PC 的带宽 ÷ 每帧字节数。**

这条链路的带宽实测在 **45~90 KB/s** 之间随 WiFi 环境波动(同一块板子、同样设置,
不同时段能差一倍), 所以"优化帧率"实际上就是两件事: **把每帧变小** 和
**把带宽用满**。

### 1. 扩展自己先卡了一道 (最大的一截)

`CAMERA_STREAM_INTERVAL_MS` 原本是 80ms —— 那是"板子两帧之间的最小间隔", 等于
**自己先把帧率锁死在 12.5 帧/秒**。而实测 QVGA 在 20ms 间隔下能跑到 17.8 帧/秒,
也就是说这道限制白白吃掉了一截。改成 **30ms**。

### 2. 视频用更小的帧

预览不需要照片那么细。加了「视频质量」积木(默认 **35**): 点「打开摄像头」时临时
切过去, 关闭时把"拍照质量"还回来 —— 这样照片还是清晰的 20, 视频帧却小了一半。
实测同样 VGA, 质量 20/35/45 分别约 14KB/10KB/8.2KB, 帧率跟着 3.7 → 5 → 7 帧/秒。

### 3. 固件: 分片合并成批量写

一片只有 246 字节, 原来一片一次 `s_send()` —— 一帧 VGA 要 70 多次 `send()`,
而且 socket 开了 `TCP_NODELAY`, 每次都会变成一个 ~250 字节的 TCP 段。段一多,
每字节要摊的 IP/TCP/WiFi 头开销就上去了。现在先把若干片拼进一个 2KB 的批
(里面仍然是若干个完整的"长度+包"结构, 协议没变), 满了再一次性发 —— 一帧只剩
4~8 次 `send()`, TCP 段也从 ~250 字节变成 ~1.4KB(正好一个 MSS)。

### 最终实测 (经完整链路: 网关拼帧 + base64 + WebSocket)

| 设置 | 帧大小 | 帧率 |
| --- | --- | --- |
| QVGA + 视频质量 35 | ~3.2 KB | **14 帧/秒** |
| VGA + 视频质量 45 | ~8.2 KB | **7 帧/秒** |
| VGA + 视频质量 35 | ~10 KB | **5 帧/秒** |
| VGA + 拍照质量 20 (老默认) | ~14 KB | 3.7 帧/秒 |

### 试过但没用的

* **继续降质量**: 收益递减(质量 45 以上画面开始糊, 帧率只再多一点点), 而且
  已经到链路带宽的天花板;
* **抬 lwIP 发送窗口**: 见上一节, 32KB 会把板子搞崩, 已回退;
* **关 WiFi 省电**: 本来就是关的(`esp_wifi_set_ps(WIFI_PS_NONE)`);
* **18ms 的往返延迟**: 这是链路给的, 除非换路由器/换 5GHz, 否则动不了。

> 想再往上走只有两个方向: **(a)** 板子侧继续压小帧(加 QQVGA 之类的档位),
> **(b)** 把 TCP 发送窗口 / 分段池一起调(先开 PSRAM 再试)。现在 QVGA 14 帧/秒
> 对"机器人摄像头预览"够用了。

## 开 PSRAM 才是真正的那把钥匙 (2026-09-24 下午)

上面那句"先开 PSRAM 再试"当天就验证了 —— **这块板子本来就有 8MB PSRAM, 只是
固件没开**。esptool 一眼就能看出来:

```
$ python -m esptool --port COM14 flash_id
Chip is ESP32-S3 (QFN56) (revision v0.2)
Features: WiFi, BLE, Embedded PSRAM 8MB (AP_3v3)      <- 8MB Octal PSRAM (N16R8)
```

而 `sdkconfig` 里是 `# CONFIG_SPIRAM is not set` —— 纯粹是没打开。

### 怎么开 (以及那个坑)

```
CONFIG_SPIRAM=y
CONFIG_SPIRAM_MODE_OCT=y          # 8MB 的是 Octal; 2MB 的那种才是 Quad
CONFIG_SPIRAM_SPEED_80M=y
CONFIG_SPIRAM_USE_MALLOC=y
CONFIG_LWIP_TCP_SND_BUF_DEFAULT=32768
```

**坑**: 打开 SPIRAM 会自动带上 `STDATOMIC_S32C1I_SPIRAM_WORKAROUND`, 它给整个
工程加 `-mdisable-hardware-atomics`, 于是 GCC 不再内联原子操作, 改成调
`__atomic_compare_exchange_4` 之类; 这些符号由 newlib 里一个专门的文件提供,
而**增量构建不会重新编它** —— 结果链接阶段报一屏
`undefined reference to __atomic_compare_exchange_4`。
解决: `idf.py fullclean` 之后重新编译 (本工程因为 managed_components 打过补丁,
fullclean 会被保护拦住, 那就先手动删掉 `firmware/build` 目录)。

### 开了之后的变化

启动日志:

```
I (232) esp_psram: Found 8MB PSRAM device
I (232) esp_psram: Speed: 80MHz
I (665) esp_psram: SPI SRAM memory test OK
I (677) esp_psram: Adding pool of 8192K of PSRAM memory to heap allocator
I (1101) cam_hal: Allocating 100000 Byte frame buffer in PSRAM   <- 帧缓冲搬走了
```

1. **相机帧缓冲搬到 PSRAM**, 内部 RAM 一下腾出 100KB 给 WiFi / lwIP;
2. 内部 RAM 宽裕之后, 上一节把板子搞崩的
   `CONFIG_LWIP_TCP_SND_BUF_DEFAULT=32768` **这次能用了**;
3. 往返延迟从 **18ms 降到 12ms**。

### 实测对比 (直连板子, QVGA 质量 35, 间隔 30ms)

| | 吞吐 | 帧率 |
| --- | --- | --- |
| 开 PSRAM 之前 | 39~96 KB/s | 10~16 帧/秒(抖) |
| 开 PSRAM + 32KB 发送窗口 | **110~129 KB/s** | **16.1/16.3/16.1 帧/秒**(连续三段) |

大帧更能看出差距: VGA 质量 20 (约 35KB/帧) 跑到 **272 KB/s** —— 而开 PSRAM 之前
这条路的上限只有 88 KB/s。连续跑三段不再掉速, 结束后回环仍然正常
(之前那样把窗口硬拉到 32KB 会"跑一会儿就死")。

经完整链路 (网关 + base64 + WebSocket) 实测:

| 设置 | 帧率 |
| --- | --- |
| QVGA + 视频质量 35 | **10.7 帧/秒** (板子实际出了 17.5 帧/秒, 中间被 PC 侧管道吃掉一部分) |
| VGA + 视频质量 45 | **9.1 帧/秒** |

也就是说瓶颈已经**从板子挪到了 PC 侧管道**(网关拼帧 → Banyan → wsgw → WebSocket)。
下一步如果还要提速, 该动的是那一段, 而不是再多给 TCP 缓冲。

## 视频"周期性卡顿" (2026-09-25): 是帧太大把链路喂满了

现象: Scratch 舞台上画面会一顿一顿地卡。先量后改 —— 新增
`tools/sniff_camera_stream.py`, 它能选在两个位置量帧到达节奏:

* `--via banyan` (默认): 网关发到 Banyan 出口的时刻;
* `--via ws`: 接 `ws://127.0.0.1:9007`, **和 Scratch 扩展收到的是同一条流**;
* 输出帧间隔的中位/p90/max、长停顿次数与时刻、自相关主周期, `--dump` 还能把
  逐帧 CSV 写下来; `--start` 可以自己开流 (结束会 `camera_stop`, `--restore`
  则把设置恢复回去)。

```powershell
python tools\sniff_camera_stream.py --seconds 15                    # 被动看正在跑的流
python tools\sniff_camera_stream.py --via ws --start --size QVGA --quality 35 --interval 30 --seconds 20
```

### 实测 (全部走 WebSocket, 也就是 Scratch 真正拿到的节奏)

| 设置 | 每帧 | 帧率 | 帧间隔 中位 / p90 / 最长 | 备注 |
| --- | --- | --- | --- | --- |
| **SXGA + 质量 10** (当时的设置) | ~79 KB | **4.7 帧/秒** | 193 / 296 / 2186 ms | 300~500ms 的停顿不断, 极端一次冻 2.2 秒 |
| VGA + 质量 35 | ~11.7 KB | 17.1 帧/秒 | 60 / 88 / 240 ms | 偶尔一次 0.2 秒 |
| QVGA + 质量 35 | ~5 KB | 28.9 帧/秒 | 22 / 90 / 206 ms | 会两三帧挤在一起到达 |
| QVGA + 质量 35 + 间隔 120ms | ~5.6 KB | 8.2 帧/秒 | 121 / 159 / 206 ms | **零长停顿**, 画面最稳 |

### 结论

`帧率 ≈ 带宽 ÷ 每帧字节数` 这条原则在"帧大到喂满链路"时就不再是"帧率低一点"
而已, 而是**变成卡顿**: 板子一直在发, 下一帧必须等上一帧发完, 于是 JPEG 体积
一波动就直接变成帧间隔波动; 碰上一个特别大的帧或者 WiFi 重传, 就是零点几秒
的冻结。当时 SXGA+质量10 单帧 79KB (最长 95KB), 实测链路吞吐约 350~400KB/s,
正好卡在这个天花板下 —— 每帧传输耗时中位 128ms, 而帧间隔中位只有 193ms。
反过来看最长的 8 个间隔, 对应的全是 80~95KB 的大帧, 相关性肉眼可见。

最后一行的对照也很说明问题: 同样 QVGA 质量 35, 把间隔从 30ms 放到 120ms
就**一点停顿都没有了** —— 把板子逼到极限 (30ms 间隔 = 33 帧/秒) 时, 板子、
WiFi、网关任何一处的小抖动都会漏到画面上; 留出余量比追最高帧率更不容易卡。

### 所以怎么改

1. Scratch 里「摄像头尺寸」用 **QVGA** (预览够用) 或 VGA; 「视频质量」35~45
   (注意语义: **数字大 = 帧小**)。
2. **别在开着摄像头的时候再设「拍照质量」/「摄像头尺寸」** —— 照片和视频共用
   板子上那一套参数, 流式播放期间设它会顺手把视频的分辨率/质量也改掉
   (当时板子就是被改成了 SXGA + 质量 10)。拍照参数放在「打开摄像头」之前设。
3. 想更稳就把流间隔放到 100~120ms (现在扩展里写死 30ms,
   `CAMERA_STREAM_INTERVAL_MS`)。

代码层面还能再做的两件事 (都还没做):

* 扩展的「打开摄像头」目前只临时替换**质量**, 不替换**尺寸**。照抄同一套逻辑,
  开流时把尺寸压到 VGA 以内、关闭时恢复, 这类卡顿就不会再出现;
* 固件 `CONFIG_TMX_CAMERA_FB_COUNT=1`: 采集和发送是串行的 (发这一帧的时候
  传感器不能往第二块缓冲里写)。大帧时改成 2 可以让两者重叠, 间隔更均匀 ——
  要重新编译烧录。

> **坑**: 量流式播放时别同时抓串口。`tools/serial_tail.py` 打开 COM14 就会让
> USB-Serial-JTAG 把板子复位, 正在跑的流会断, 网关也会被守护进程重启
> (`supervise_s3extend.ps1` 会自动重连板子, 但要重新点一次「打开摄像头」)。

## 画面"突然定格"的两个根因 (2026-09-25 晚): 单帧出错不再停整条流

现象: 画面定格不动, 但板子还在出图 (实测帧一直在到、每一张 JPEG 都能正常解码、
扩展也确实在画 —— 改一下「摄像头尺寸」舞台立刻跟着变)。查下来是扩展里有
两个"一次性致命"的设计:

1. `handleVideoFrame` 的 catch 里直接 `videoOn = false` —— **任意一帧**在解码/
   换贴图这条路上抛异常, 整条流就被永久停掉, 而且不给板子发 `camera_stop`:
   板子继续出图发热, 画面却定格。最容易踩到的时候是**中途改分辨率/换角色**
   (画布尺寸变了) 之后的那一帧。
2. `openVideo()` 开头是 `if (this.videoOn) return;` —— 卡住之后**再点一次
   「打开摄像头」完全没反应**, 只能先去点「关闭摄像头」。

### 改法 (scratch/esp32s3.js)

* **单帧失败只丢这一帧**: `videoErrors` 计数, 提示"这一帧没画出来（原因），继续"，
  **连续 `CAMERA_VIDEO_MAX_ERRORS`(=5) 帧**都失败才停流, 并在提示里写明
  "再点一次「打开摄像头」可以重开"; 画成功一帧就把计数清零。
* **「打开摄像头」再点一次 = 重开**: 清状态 + 重新发 `camera_config`/`camera_snapshot`。
  注意重开时**不能覆盖** `qualityBeforeStream`, 否则关流时会把"视频质量"当成原来的
  拍照质量还回去。
* **看门狗**: 每秒跑一次 `videoWatchdog()`。帧一直在来、却连续
  `CAMERA_VIDEO_STALL_MS`(=3 秒) 没画成功 → 判定卡在"上一帧还没画完"上
  (某个 promise 一直不 settle), 清掉忙标记并把手里最新一帧补画上去, 自动恢复
  (`videoRecoveries` 记次数, 状态文字里也会说"正在自动重画（第 N 次）")。
  帧本身不来 (链路断了) 时它不动手 —— 那种情况重画也没用。
* 服务断开 (`onclose`) 时把新增的这些状态一起复位。

测试: `tools/test_esp32s3_camera.js` 增加了四组 —— 单帧失败不停流、连续 5 帧才停、
重开有效且不覆盖拍照质量、看门狗补画。`node tools\test_esp32s3_camera.js` 全部通过。

### 顺带记一笔: "看起来卡住"其实可能只是场景没动

同一天还量到一种情况: 连续 15 秒的 87 帧里, 相邻帧**平均只差 1.8 个灰阶**,
变化超过 12 的像素只占 **1.5%** —— 板子、链路、扩展全都正常, 只是镜头前那 15 秒
没有东西在动 (画面本身是正常场景: 平均亮度 126、标准差 59)。

**一眼分辨的办法**: 改一下「摄像头尺寸」或「视频质量」——舞台如果立刻跟着变,
说明扩展一直在画, 那就是"场景本身静止"; 如果舞台纹丝不动, 才是真卡住。

### 还有一根刺: 每帧多拷一份全画面 —— 渲染进程涨内存, 最后软件崩

现象: 画面卡顿/不动的同时 TurboWarp 越来越吃内存, 最后整个崩掉 (进程直接没了,
Windows 事件日志里连崩溃记录都留不下)。实测**渲染进程 30 秒涨 +96MB**
(约 3MB/s), GPU 进程同期 +58MB。

原因在 scratch-render 的 `BitmapSkin.setBitmap()` 里 (从 TurboWarp Desktop 的
app.asar 里翻出来的):

```js
setBitmap(img, resolution, rotationCenter) {
    ...
    if (img instanceof HTMLCanvasElement && img.reusable !== false) {
        img = img.getContext('2d').getImageData(0, 0, img.width, img.height);   // 全画面拷贝
    }
    ...
}
```

流式播放每帧都新建一个 canvas 交给 `updateBitmapSkin()`, 但**没标 `reusable`**,
于是每帧都被拷一份全画面像素: XGA(1024x768) 3MB、SXGA(1280x1024) 5MB, 6 帧/秒就是
每秒十几 MB 的垃圾 —— 内存一路涨, 几分钟后 TurboWarp 崩。TurboWarp 自己的
Sprite2 / 位图路径交 canvas 给渲染器时, 都会先 `canvas.reusable = false`。

修法就一行: `decodeJpegToCanvas()` 建好画布后标 `canvas.reusable = false`, 渲染器
就直接把画布上传成纹理, 不再拷贝 (顺带省掉每帧一次全画面 CPU 读回)。
`tools/test_esp32s3_camera.js` 里加了断言: 交给渲染器的画布必须 `reusable === false`。

> 重载扩展后可以这样对比: 流开着时每秒看一眼 TurboWarp 各进程的内存
> (`Get-Process TurboWarp | Select Id,@{n='MB';e={[int]($_.WorkingSet64/1MB)}}`),
> 渲染进程应该不再随帧数线性上涨。

#### 后续: 真正的"大头"是每帧新建画布 (GPU 进程内存 30~40MB/s)

加上 `reusable=false` 之后渲染进程的内存确实不再线性上涨了, 但 TurboWarp 的
**GPU 进程**内存还在以 **30~40MB/s** 的速度涨, 一直涨到 ~2.8GB 才回收一次
(9 帧/秒 × XGA 3MB/帧 ≈ 27MB/s, 对得上)。做了个 A/B 确认这就是视频造成的:

```
流开着:   gpu-process 953MB -> 1016 -> 1108 -> 1345 -> 1544 -> 1755   (每 5 秒一采)
发 camera_stop 后 (已确认帧停了):  1239MB -> 988 -> 988 -> 988 -> 988   (稳住了)
```

原因是流式播放**每帧都 `document.createElement('canvas')`**: 每块 XGA 画布都要
一份 3MB 的画布后备存储 (走 GPU), 于是显存一路涨。改法: 流式播放复用同一块画布
(`v_streamCanvas`), 只在分辨率变了才改 `width/height`;

* 安全性依据: scratch-render 的 `BitmapSkin._setTexture()` 是**同步** `texImage2D`,
  上传完就可以覆盖这块画布 (轮廓/碰撞检测会读到最新内容, 本工程用不到);
* 拍照/截图那条路仍然每次新建画布 (要拿去做 PNG 编码, 不能和视频抢同一块);
* 测试里加了断言: 连画 3 帧只允许 new 1 块画布。

### 一条测量经验: 别用屏幕截图判断"画面有没有在动"

TurboWarp 窗口被别的窗口挡住 / 最小化时, Chromium 会**停止重绘那个窗口** ——
这时不管视频在不在跑, 截图都看不出变化; 反过来, 如果在别的窗口(比如 VS Code)
前面做截图, 拍到的其实是前面那个窗口。判断"扩展有没有在画"要用进程指标:

* 流开着时渲染进程 CPU 有明显占用 (实测 9 帧/秒 XGA 时约 40% 单核);
* 或者改一下「摄像头尺寸」/「视频质量」, 看舞台有没有跟着变。

### 调试上报: 让扩展自己把状态说出来 (2026-09-25 晚)

截图、串口、进程指标都各有前提条件, 最省事的还是让扩展自己上报。扩展里的
`VIDEO_DIAG_ENABLED`(默认 `true`) 打开时, 每 5 秒往 Banyan 总线发一条
`{"command": "video_diag", ...}`, 网关对不认识的命令直接忽略 (不会打扰板子),
所以这条路是安全的。里面有:

```
on / busy / frames / dropped / errors / recoveries / fps
since_arrival_ms        距上一帧到达多久
since_draw_ms           距上次画成功多久  <- 帧在来但它很大 = 卡在"画不出来"
costume / costume_skin  正在刷的那个造型 (名字 + skinId)
current_costume         角色当前显示的造型  <- 和 costume 不一致 = 画了但没显示
sprite / visible / size 当前编辑目标 / 是否可见 / 大小
costumes                该角色的全部造型名
```

PC 侧这样读:

```powershell
python tools\sniff_camera_stream.py --diag --seconds 20
```

排查完把 `VIDEO_DIAG_ENABLED` 改回 `false` 即可。

### 「画面不动」的真凶: 造型被复制过 (2026-09-25 深夜)

上面那两行诊断字段一上来就把它抓出来了:

```
当前编辑目标 = 角色1, 造型 = 摄像头画面2
「摄像头画面」造型: 存在=摄像头画面 skinId=2
该角色的全部造型: ['造型1', '摄像头画面', '摄像头画面2']
```

角色当前显示的是 **「摄像头画面2」**(造型被复制过, TurboWarp 自动加序号),
而扩展每帧刷的是 **「摄像头画面」** —— 扩展画得再勤, 舞台上显示的也是那张静止的
复制品。于是所有现象都对上了: 帧一直在到、渲染进程也在忙 (在画)、截图看不出变化、
改画质舞台也不变。**画面不是"卡住", 而是根本没显示正在刷的那块造型。**

改法: 每次画完调用 `showVideoCostume()` 确认角色当前造型就是正在刷的那一块,
不一致就切回来 (scratch-vm 的 `currentCostume` setter 会同步给渲染器), 并在状态
文字里说一句「已切回「摄像头画面」造型（原来显示的是别的造型）」。
测试: 假 VM 里塞一个「造型1」并设为当前造型, 断言下一帧会把造型切回来。

### 还有一种"画面不动": 本地 ws 网关不往外发了 (2026-09-25 深夜)

调试上报显示得非常干脆:

```
流式播放: 开着=True 已画=0 帧 ... 收到上报 1318 条, 最近一条 = camera_status (121283 ms 前)
```

也就是说: 板子在出图 (总线上 8 帧/秒)、扩展自己也认为在放, 但它**已经两分钟没收到
任何上报**了。为了排除"扩展收不到大帧"的可能, 直接往 `from_esp32_gateway` 上
重发了一整帧 `camera_frame` —— 计数纹丝不动, 确认是**接收通道断了**; 而它那条
WebSocket 在 TCP 层还是 ESTABLISHED。

结论: 卡在本地 `wsgw` 这一环。`ws_gateway.py` 的 `incoming_message_processing()`
是 `for ... await send()` 且不兜异常, 客户端一多 (每次都连一下的诊断工具、反复
重载的扩展) 就容易把转发循环搞死 —— 表现正是"板子在出图、网关日志写着已连接、
Scratch 里画面却不动"。**重启 wsgw 立刻恢复** (实测: 重启后画面就回来了)。

已加**补丁 12** (`tools/apply_local_patches.py`): 发送失败只丢掉那个客户端,
继续给其他人发, 并打印一行日志。补丁已应用到已安装的包, 下次 wsgw 重启后生效。

#### 「画面不动」的一分钟恢复手册

1. `python tools\sniff_camera_stream.py --diag --seconds 15` 看 `已画` 和
   `收到上报` 这两个数有没有在涨;
2. 如果 `收到上报` 不涨 (接收通道断了) → 重启本地服务:
   ```powershell
   Get-Process wsgw   -ErrorAction SilentlyContinue | Stop-Process -Force
   Get-Process esp32gw -ErrorAction SilentlyContinue | Stop-Process -Force
   ```
   守护进程几秒内会把它们拉起来;
3. 在 Scratch 里点一次「连接板子 IP」→「打开摄像头」;
4. 如果 `已画` 在涨但画面还是不动 → 看 `当前造型` 是不是「摄像头画面」,
   不是的话就是造型被复制/切走了 (已由 showVideoCostume 兜住)。

### 在线改分辨率不生效: 必须让摄像头重新上电 (2026-09-25 深夜)

现象: 流式播放中改「摄像头尺寸」, 画面大小/分辨率不变。

实测 (`camera_config size=N` 之后直接量帧的像素尺寸):

```
1 现在            : 自报=640x480(VGA)   帧=640x480
2 在线改 QVGA     : 自报=320x240(QVGA)  帧=640x480   <- 设置改了, 帧没变
3 停流→改 SXGA→重开: 自报=1280x1024     帧=1280x1024  <- 才生效
4 停流→改 VGA→重开 : 自报=640x480       帧=640x480
```

原因: 固件里"在线改尺寸"只是 `sensor->set_framesize()`, 而 DVP/JPEG 这条采集
通路是 `esp_camera_init()` 时按当时的分辨率配好的 —— 只改传感器寄存器不够,
必须 `camera_sensor_off()` (deinit + 断电) 之后重新 `tmx_camera_init()`
(`camera_config_t.frame_size` 取自当时的 `s_size_index`) 才会按新尺寸采集。

所以「摄像头尺寸」积木现在的做法是: 如果在放视频, 就**停流 → 重新开流**
(即让摄像头重新上电一次), 用户看到的是"停顿 2~3 秒预热, 然后画面按新尺寸出来"。
另外舞台上的画面大小 = 造型像素 ÷ 2 (bitmapResolution) × 角色"大小"百分比,
QVGA/VGA/SXGA 分别约 160x120 / 320x240 / 640x512 个舞台单位。

**2026-09-25 晚已修在固件里**(并烧录验证): `tmx_camera_set_format()` 发现
`frame_size` 真的变了, 就自己 `drop_current_frame()` → `camera_sensor_off()` →
`tmx_camera_init()` 把采集通路重建一遍。实测在线切 SXGA/QVGA/VGA 都是下一帧就
跟着变 (中间 2~3 秒在重建 + 预热), 扩展侧不再需要"停流再重开"那套绕法。

同一版固件还打开了 `CONFIG_LWIP_TCP_KEEPALIVE` —— 之前代码里给 client socket
设的 TCP_KEEPIDLE/INTVL/CNT 其实被 lwIP 忽略, 客户端"半死"(进程被杀/掉电, 没发
FIN)时板子会永远等在旧 socket 上, 新连接进不来 (表现为 ping 通、TCP 连不上,
只能复位板子)。打开之后大约 25 秒就能把这半死连接回收掉。

## 剩下的"周期性卡顿": 是**电脑的无线网卡每 6 秒重关联一次** (2026-09-26)

现象: 板子、固件、扩展都正常了, 视频仍然每 **~6 秒**冻结 **1.2~1.5 秒** (XGA
10.3 帧/秒时最明显)。

### 定位过程 (一路排除到电脑的网卡)

1. **板子侧没问题**: RSSI **-45dBm** (信号很强)、信道 **6** (扫到的 9 个邻居全在信道
   11)、`Set ps type: 0` (省电关闭)、91/91 帧合法且没有拼帧丢失。
2. **两个方向同时卡** → 不是板子: `ping 路由器` (5GHz 直连) 和 `ping 板子` 在**同一
   时刻**一起卡 (路由器那条从 0% 丢包变成 4.5%、尖峰 550ms)。板子之外的公共环节
   只有电脑自己。
3. **WLAN 事件日志** (`Microsoft-Windows-WLAN-AutoConfig/Operational`) 给出铁证:
   每 **6 秒**一次 `无线安全功能 已停止 → 已启动 → 成功` (11004/11010/11005), 而
   ping 的丢包/尖峰**全部紧跟在这些事件之后** (例: :10 事件后 :12 出现 1074ms、
   :14~:16 连续丢包)。
4. **网卡高级属性** (`Get-NetAdapterAdvancedProperty -Name WLAN`):

   ```
   Multi-Channel Concurrent         = Enabled + Hotspot
   Concurrent Operation Preference  = 2.4GHz Single Channel Operation
   ```

   驱动被配置成"STA+热点并发、偏好 2.4GHz 单信道", 而路由器 "zhr" 是 2.4G/5G
   同名 (会做频段引导) —— 两边互相拉锯, 结果就是每 6 秒重新关联/重协商密钥一次。

### 修法 (管理员 PowerShell; 改完网卡会自动重启一下)

```powershell
Set-NetAdapterAdvancedProperty -Name WLAN -DisplayName 'Multi-Channel Concurrent' -DisplayValue 'Disabled'
Set-NetAdapterAdvancedProperty -Name WLAN -DisplayName 'Concurrent Operation Preference' -DisplayValue 'No Preference'
Restart-NetAdapter -Name WLAN
```

(更省事的替代方案: 给电脑插网线。当时 `以太网` 是 Disconnected, 整条无线链路都能绕开。)

### 改前 / 改后

| | 改之前 | 改之后 |
| --- | --- | --- |
| WLAN 安全重关联 | **每 6 秒一次** | **3 分钟内 0 次** |
| 视频帧间隔 | 中位 120ms, p90 203ms, 最长 **1547ms**, 长停顿 4 次 / 13 秒 | 中位 120ms, p90 136ms, 最长 **232ms**, **长停顿 0 次** |
| 帧完整性 | 全部合法 | 171/171 全部合法 |
| `ping 路由器` | 0% → 4.5% 丢包, 尖峰 550ms | 0% 丢包, RTT 1.6ms |

### 两个记下来的坑

* **板子推流时不能拿 ping 判断视频好坏**: 视频在跑时板子的 ICMP 回包排在 TCP
  视频数据后面, 实测 `ping 板子` 中位 157ms、偶尔 900ms —— 但同期视频帧间隔
  中位 120ms、零长停顿。判断视频质量要看**帧到达间隔/长停顿**(用
  `tools/sniff_camera_stream.py`), 别被 ping 带偏。
* 这类"两个方向一起卡"的现象, 先怀疑**电脑自己的无线网卡** (周期性扫描 / 重关联 /
  频段引导拉锯), 用 `ping 路由器` 和 WLAN 事件日志两条线一对就能确认。

## 板子重启 / 换电池供电之后画面定格 (2026-09-26 晚)

现象: 改成电池供电之后摄像头画面不动了。

查到的顺序:

1. 板子**在线**但**没有帧**: `ping` 通、网关 TCP 也是 ESTABLISHED、`camera_info`
   回的是开机默认值 (`640x480 / 质量 20`), 而总线上一条 `camera_frame` 都没有;
2. 板子的 IP 从 `192.168.0.100` 变成了 **`192.168.0.103`** (UDP 广播
   `ONEGPIO 192.168.0.103 31336` + 网关自动发现), 电脑本机地址也从 .107 变成了 .101;
3. 结论: **换电源那一下板子断电重启了**。重启后它自己回到"空闲"(摄像头断电、
   没有流), 而扩展那边 `videoOn` 还是 true —— 于是画面一直定格, 只能人工再点一次
   「打开摄像头」。

修法 (扩展侧, 2026-09-26): 看门狗里补了一条"连着但很久没有画面就自己重开流"——
`wantVideo` 还开着、socket 也是通的、但已经 **8 秒**没有新帧时, 自动重发
`camera_config` + `camera_snapshot`; **10 秒最多一次** (板子上电 + 预热本身要 3 秒,
重发太密反而把它打断)。这样换电源 / 板子复位 / OTA 重启之后画面会自己回来。

顺带记下这次的两个坑:

* 换电源(或复位)之后板子的**分辨率/质量会回到开机默认值** (Kconfig 的 VGA/质量 20),
  用户在 Scratch 里点一次「摄像头尺寸」即可 (新固件在线生效, 中间停 2~3 秒);
* 板子换了 IP 之后, 记得把「连接板子 IP」积木里的地址也改掉 (网关有自动发现会自己
  连对, 但积木里的旧地址再点一次会把网关指到错的设备上)。
